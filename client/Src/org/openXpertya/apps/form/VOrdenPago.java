...
//		jTabbedPane2.setSelectedIndex(TAB_INDEX_EFECTIVO);
	...
	protected void updatePayAmt(BigDecimal amt){
		...
		/*
		 * Si trabajo con diferente monedas, entonces el total se expresara como la suma de todos
		 * los valores en dichas monedas 
		 */
		if(m_C_Currency_ID != currencyID) {
			float total = 0;
			if((getModel().m_facturas) != null) {
				for(ResultItem r: getModel().m_facturas) {
					ResultItemFactura f = (ResultItemFactura) r;					
					if((((BigDecimal)r.getItem(9)).floatValue() != ((BigDecimal)r.getItem(7)).floatValue())){
						total += f.getManualAmount().floatValue();
					} 
				}
				amt = new BigDecimal(total);
			}
		}

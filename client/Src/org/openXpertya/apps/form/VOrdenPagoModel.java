		...
		if (m_facturas != null) {
			for (ResultItem x : m_facturas) {
				if (((ResultItemFactura) x).getManualAmtClientCurrency()
					...
					double exchange = getConversionRateByInvoice((Integer)x.getItem(0));
...
//			rif.setManualAmtClientCurrency(openAmt);
		...
		BigDecimal manualAmt = MCurrency.currencyConvert(
				...
				getCtx());
		if(manualAmt == null) {
			Double exchange = getConversionRateByInvoice((Integer)rif.getItem(0));
			manualAmt = new BigDecimal(manualAmtClientCurrency.floatValue() / exchange);
		}
...
//			rif.setManualAmount(openAmt);
		...
		if(manualAmtClientCurrency == null) {
			Double exchange = getConversionRateByInvoice((Integer)rif.getItem(0));
			manualAmtClientCurrency = new BigDecimal(exchange * manualAmt.floatValue());
		}
	...
	/**
	 * Extrae la cotizacion desde la factura
	 * Ete metodo suele reemplazar al MCurrency.currencyConvert
	 **/
	public Double getConversionRateByInvoice(int invoiceId){
		/* Actualizo el campo de valor en ARS si es que la factura esta en otra moneda
		 * Utilizo la cotizacion dada colocada en la factura */
		String sqlId = "(SELECT exchangeRate FROM C_Invoice  WHERE c_invoice_id=?)";
		PreparedStatement pstmt = null;
		double exchange = 0.00;
		try {
            pstmt = DB.prepareStatement(sqlId);
            pstmt.setInt(1, invoiceId); //ID de invoice
            ResultSet result = pstmt.executeQuery();

            if(result.next()) {
                exchange = result.getDouble(1); //unica columna que trae
            }
            ...
            log.log(Level.SEVERE, "getRate", e);
        }
		if(exchange == 0){ // Si la cotizacion es 0 => es xq esta trabajando en misma moneda. El tipo de cambio es 1
			return 1D;
		}		
		return exchange;
	}
